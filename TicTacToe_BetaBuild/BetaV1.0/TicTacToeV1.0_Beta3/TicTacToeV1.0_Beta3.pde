...
void parseGamesave() {
  ...
  int j = 0; // initial counting value for O's indexs while loop
    while (i<=3){ // recorver data for each cellState_O
      cellState_O[i][1] = int(pieces[j]);
      cellState_O[i][2] = int(pieces[j+1]);
      cellState_O[i][3] = int(pieces[j+2]);
      i = i+1;
      j = j+3;
    }
    int x = 1; // initial counting value for X's indexs while loop
    while (x<=3){ // recorver data for each cellState_X
      cellState_X[x][1] = int(pieces[j]);
      cellState_X[x][2] = int(pieces[j+1]);
      cellState_X[x][3] = int(pieces[j+2]);
      x = x+1;
      j = j+3;
    }
  gameState = boolean(pieces[18]);
  gameTie = boolean(pieces[19]);
  turn = int(pieces[20]);
  clickCount = int(pieces[21]);
  playerHint = int(pieces[22]);
  i = int(pieces[23]);
...
void draw(){ // draw game board, O's and X's, print game state.
  ...
  if (gameState == true){
    if ((mouseX>0 && mouseX<200) && (mouseY>0 && mouseY<200) && (cellState_X[1][1] != 1 && cellState_O[1][1] != 1)){
      drawShadow(100,100,150,150);}
    if ((mouseX>200 && mouseX<400) && (mouseY>0 && mouseY<200) && (cellState_X[1][2] != 1 && cellState_O[1][2] != 1)){
      drawShadow(300,100,350,150);}
    if ((mouseX>400 && mouseX<600) && (mouseY>0 && mouseY<200) && (cellState_X[1][3] != 1 && cellState_O[1][3] != 1)){
      drawShadow(500,100,550,150);}
    if ((mouseX>0 && mouseX<200) && (mouseY>200 && mouseY<400) && (cellState_X[2][1] != 1 && cellState_O[2][1] != 1)){
      drawShadow(100,300,150,350);}
    if ((mouseX>200 && mouseX<400) && (mouseY>200 && mouseY<400) && (cellState_X[2][2] != 1 && cellState_O[2][2] != 1)){
      drawShadow(300,300,350,350);}
    if ((mouseX>400 && mouseX<600) && (mouseY>200 && mouseY<400) && (cellState_X[2][3] != 1 && cellState_O[2][3] != 1)){
      drawShadow(500,300,550,350);}
    if ((mouseX>0 && mouseX<200) && (mouseY>400 && mouseY<600) && (cellState_X[3][1] != 1 && cellState_O[3][1] != 1)){
      drawShadow(100,500,150,550);}
    if ((mouseX>200 && mouseX<400) && (mouseY>400 && mouseY<600) && (cellState_X[3][2] != 1 && cellState_O[3][2] != 1)){
      drawShadow(300,500,350,550);}
    if ((mouseX>400 && mouseX<600) && (mouseY>400 && mouseY<600) && (cellState_X[3][3] != 1 && cellState_O[3][3] != 1)){
...
void keyPressed(){// for resetting or saving & closing a game
  ...
  if (key == 's' || key == 'S'){ // saving a game
    ...
    int i = 1; // initial counting value for O's indexs while loop
    while (i<=3){ // print O's indexs
      output.println(cellState_O[i][1]);
      output.println(cellState_O[i][2]);
      output.println(cellState_O[i][3]);
      i = i+1;
    }
    int j = 1; // initial counting value for X's indexs while loop
    while (j<=3){ // print X's indexs
      output.println(cellState_X[j][1]);
      output.println(cellState_X[j][2]);
      output.println(cellState_X[j][3]);
      j = j+1;
    }
...
void mousePressed(){
  fill(255);   
  if (gameState == true){
    if (turn == 1){ // turn = 1 is O's turn, assign mouseCode for animation, append cellState_O list, swap turns and increase clickCount value.
      circleRad = 100; 
    ...
    if (turn == 2){ // turn = 2 is X's turn, assign mouseCode for animation, append cellState_X list, swap turns and increase clickCount value.
      lineLength = 0;
  ...
  while (i<=3){
    if (turn == 1){
      if (cellState_O[i][1] == 0 && cellState_X[i][1] == 0 && cellState_O[i][2] == 1 && cellState_O[i][3] == 1){
        playerHint = 1;}
      if (cellState_O[i][1] == 1 && cellState_O[i][2] == 0 && cellState_X[i][2] == 0 && cellState_O[i][3] == 1){
        playerHint = 1;}
      if (cellState_O[i][1] == 1 && cellState_O[i][2] == 1 && cellState_O[i][3] == 0 && cellState_X[i][3] == 0){ 
        playerHint = 1;}
    }
    if (turn == 2){
      if (cellState_X[i][1] == 0 && cellState_O[i][1] == 0 && cellState_X[i][2] == 1 && cellState_X[i][3] == 1){
        playerHint = 2;}
      if (cellState_X[i][1] == 1 && cellState_X[i][2] == 0 && cellState_O[i][2] == 0 && cellState_X[i][3] == 1){
        playerHint = 2;}
      if (cellState_X[i][1] == 1 && cellState_X[i][2] == 1 && cellState_X[i][3] == 0 && cellState_O[i][3] == 0){ 
        playerHint = 2;}
    }
    i = i + 1;
  }
  ...
  while (i<=3){
    if (turn == 1){
      if (cellState_O[1][i] == 0 && cellState_X[1][i] == 0 && cellState_O[2][i] == 1 && cellState_O[3][i] == 1){
        playerHint = 1;}
      if (cellState_O[1][i] == 1 && cellState_O[2][i] == 0 && cellState_X[2][i] == 0 && cellState_O[3][i] == 1){
        playerHint = 1;}
      if (cellState_O[1][i] == 1 && cellState_O[2][i] == 1 && cellState_O[3][i] == 0 && cellState_X[3][i] == 0){ 
        playerHint = 1;}
    }
    if (turn == 2){
      if (cellState_X[1][i] == 0 && cellState_O[1][i] == 0 && cellState_X[2][i] == 1 && cellState_X[3][i] == 1){
        playerHint = 2;}
      if (cellState_X[1][i] == 1 && cellState_X[2][i] == 0 && cellState_O[2][i] == 0 && cellState_X[3][i] == 1){
        playerHint = 2;}
      if (cellState_X[1][i] == 1 && cellState_X[2][i] == 1 && cellState_X[3][i] == 0 && cellState_O[3][i] == 0){ 
        playerHint = 2;}
    }
    i = i + 1;
  }
  ...
  // diagonal hint
  if (turn == 1){
    if (cellState_O[1][1] == 0 && cellState_X[1][1] == 0 && cellState_O[2][2] == 1 && cellState_O[3][3] == 1){
      playerHint = 1;}
    if (cellState_O[1][1] == 1 && cellState_O[2][2] == 0 && cellState_X[2][2] == 0 && cellState_O[3][3] == 1){
      playerHint = 1;}
    if (cellState_O[1][1] == 1 && cellState_O[2][2] == 1 && cellState_O[3][3] == 0 && cellState_X[3][3] == 0){ 
      playerHint = 1;}
    if (cellState_O[3][1] == 0 && cellState_X[3][1] == 0 && cellState_O[2][2] == 1 && cellState_O[1][3] == 1){
      playerHint = 1;}
    if (cellState_O[3][1] == 1 && cellState_O[2][2] == 0 && cellState_X[2][2] == 0 && cellState_O[1][3] == 1){
      playerHint = 1;}
    if (cellState_O[3][1] == 1 && cellState_O[2][2] == 1 && cellState_O[1][3] == 0 && cellState_X[1][3] == 0){ 
      playerHint = 1;}
  }
  if (turn == 2){
    if (cellState_X[1][1] == 0 && cellState_O[1][1] == 0 && cellState_X[2][2] == 1 && cellState_X[3][3] == 1){
      playerHint = 2;}
    if (cellState_X[1][1] == 1 && cellState_X[2][2] == 0 && cellState_O[2][2] == 0 && cellState_X[3][3] == 1){
      playerHint = 2;}
    if (cellState_X[1][1] == 1 && cellState_X[2][2] == 1 && cellState_X[3][3] == 0 && cellState_O[3][3] == 0){ 
      playerHint = 2;}
    if (cellState_X[3][1] == 0 && cellState_O[3][1] == 0 && cellState_X[2][2] == 1 && cellState_X[1][3] == 1){
      playerHint = 2;}
    if (cellState_X[3][1] == 1 && cellState_X[2][2] == 0 && cellState_O[2][2] == 0 && cellState_X[1][3] == 1){
      playerHint = 2;}
    if (cellState_X[3][1] == 1 && cellState_X[2][2] == 1 && cellState_X[1][3] == 0 && cellState_O[1][3] == 0){ 
      playerHint = 2;}
  }
}
